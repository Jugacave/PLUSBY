          ...
          <div className="bg-[#0A0A0F] rounded-xl p-4 space-y-2.5">
            ...
            <div className="flex justify-between items-center">
              ...
              <span
                ...
                style={{ color: calcResults.netProfit >= 0 ? marginColor(calcResults.marginPct) : "#EF4444" }}
              >
            ...
            <div className="flex justify-between items-center text-xs">
              ...
              <span className="font-bold" style={{ color: marginColor(calcResults.marginPct) }}>
                {calcResults.marginPct}%
              </span>
            </div>
            ...
            <div className="h-1.5 bg-[#2A2A3A] rounded-full overflow-hidden mt-1">
              <div
                ...
                style={{
                  width: `${Math.min(calcResults.marginPct, 100)}%`,
                  background: marginColor(calcResults.marginPct),
                }}
              />
            </div>
          </div>
            ...
            <div className="space-y-4">
              {[
                { label: "Ingresos brutos", value: formatCOP(calcResults.monthlyRevenue), color: "#F0F0F5", bar: 100 },
                {
                  ...
                  value: formatCOP(calcResults.monthlyRevenue - calcResults.monthlyProfit),
                  color: "#F59E0B",
                  bar: pct(calcResults.monthlyRevenue - calcResults.monthlyProfit, calcResults.monthlyRevenue),
                },
                {
                  label: "Ganancia neta",
                  value: formatCOP(calcResults.monthlyProfit),
                  color: "#4ADE80",
                  bar: pct(calcResults.monthlyProfit, calcResults.monthlyRevenue),
                },
              ].map((item) => (
                <div key={item.label}>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-[#8888A0] text-xs mt-0.5">
                  Necesitas vender mínimo{" "}
                  <span className="text-[#FF6B35] font-bold">
                    {calcResults.breakEven > 0 ? `${calcResults.breakEven} unidades` : "—"}
                  </span>{" "}
                  al mes para cubrir costos fijos.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-[rgba(124,58,237,0.08)] to-transparent border border-[rgba(124,58,237,0.15)] rounded-2xl p-5">
            ...
            <p className="text-[#8888A0] text-xs leading-relaxed">
              {calcResults.marginPct >= 55
                ...
                ? `Margen aceptable del ${calcResults.marginPct}%. Considera reducir el costo de envío o negociar mejor precio con tu proveedor para mejorar la rentabilidad.`
                : `Margen bajo del ${calcResults.marginPct}%. Revisa tus costos. El producto puede no ser viable con estos números. Intenta aumentar el precio o reducir costos.`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#13131A] border border-[#2A2A3A] rounded-2xl overflow-hidden">
        <div className="flex items-center justify-between p-5 border-b border-[#2A2A3A]">
          ...
          <Link
            ...
            className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-[#1C1C26] hover:bg-[#2A2A3A] text-[#8888A0] hover:text-[#F0F0F5] text-xs font-medium transition-colors"
          >
            <Package size={13} />
            Agregar producto
          </Link>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#2A2A3A]">
                {["Producto", "Precio venta", "Costo total", "Ganancia/u", "Margen", "Unid/mes", "Ganancia mensual"].map((h) => (
                  <th key={h} className="px-4 py-3 text-left text-[10px] font-semibold text-[#555568] uppercase tracking-wider whitespace-nowrap">
                    {h}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {products.map((p, i) => {
                ...
                return (
                  <tr
                    key={p.id}
                    className={`border-b border-[#1C1C26] hover:bg-[#1C1C26] transition-colors ${
                      i === products.length - 1 ? "border-0" : ""
                    }`}
                  >
                    <td className="px-4 py-3.5">
                      <p className="text-[#F0F0F5] text-sm font-medium">{p.name}</p>
                      <p className="text-[#555568] text-xs">Comisión {p.platformFee}%</p>
                    </td>
                    <td className="px-4 py-3.5 text-[#F0F0F5] text-sm font-medium whitespace-nowrap">
                      {formatCOP(p.price)}
                    </td>
                    <td className="px-4 py-3.5 text-[#8888A0] text-sm whitespace-nowrap">
                      {formatCOP(totalCost)}
                    </td>
                    <td className="px-4 py-3.5 text-sm font-semibold whitespace-nowrap" style={{ color: marginColor(margin) }}>
                      {formatCOP(netUnit)}
                    </td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                        <div className="w-12 h-1.5 bg-[#2A2A3A] rounded-full overflow-hidden">
                          <div
                            className="h-full rounded-full"
                            style={{ width: `${margin}%`, background: marginColor(margin) }}
                          />
                        </div>
                        <span className="text-xs font-bold whitespace-nowrap" style={{ color: marginColor(margin) }}>
                          {margin}%
                        </span>
                      </div>
                    </td>
                    <td className="px-4 py-3.5 text-[#8888A0] text-sm text-center">
                      {p.unitsSold}
                    </td>
                    <td className="px-4 py-3.5 text-sm font-bold whitespace-nowrap" style={{ color: marginColor(margin) }}>
            ...
            <tfoot>
              <tr className="bg-[#1C1C26] border-t border-[#2A2A3A]">
                <td className="px-4 py-3 text-[#F0F0F5] text-xs font-bold" colSpan={5}>
                  TOTAL MENSUAL ESTIMADO
                </td>
                <td className="px-4 py-3 text-[#F0F0F5] text-xs font-bold text-center">
                  {products.reduce((s, p) => s + p.unitsSold, 0)}
                </td>
                <td className="px-4 py-3 text-green-400 text-sm font-bold whitespace-nowrap">
